...
* Error: error connecting to PostgreSQL server csm-aks-dev-devops.postgres.database.azure.com (scheme: postgres): dial tcp 20.x1.xxx.2x:5432: i/o timeout
    with module.db_external_postgres[0].postgresql_role.admin,
    on modules/db_external_postgres/main.tf line 23, in resource "postgresql_role" "admin"[...]
    > Make sure the ip address of you machine is adequately whitelisted in the PostgreSQL server firewall rules. or connect through your VPN!
...
* modules
    * **terraform-tenant**
        ...
        * *db_external_postgres* = **[optional, enabled with `use_external_postgres = true`]** declaratively provision the Cosmo Tech API roles (admin/writer/reader), database, schema and privileges on an external PostgreSQL server (e.g. Azure PostgreSQL Flexible Server), using the credentials generated by `chart_postgresql`.
