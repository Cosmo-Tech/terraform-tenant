locals {
  ...
  cosmotech_api_db = var.use_external_postgres ? {
    host            = module.db_external_postgres[0].database_host
    port            = tostring(module.db_external_postgres[0].database_port)
    database        = module.db_external_postgres[0].database_name
    admin_username  = module.db_external_postgres[0].admin_username
    writer_username = module.db_external_postgres[0].writer_username
    reader_username = module.db_external_postgres[0].reader_username
    } : {
...
module "db_external_postgres" {
  source = "./modules/db_external_postgres"
...
module "chart_cosmotech_api" {
  ...
  postgresql_host            = local.cosmotech_api_db.host
  postgresql_port            = local.cosmotech_api_db.port
  postgresql_database        = local.cosmotech_api_db.database
  postgresql_admin_username  = local.cosmotech_api_db.admin_username
  postgresql_admin_password  = module.chart_postgresql.database_cosmotech_password_admin
  postgresql_writer_username = local.cosmotech_api_db.writer_username
  postgresql_writer_password = module.chart_postgresql.database_cosmotech_password_writer
  postgresql_reader_username = local.cosmotech_api_db.reader_username
  postgresql_reader_password = module.chart_postgresql.database_cosmotech_password_reader
